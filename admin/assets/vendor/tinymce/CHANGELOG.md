...
- Corrected the `role_na` attribute on listbox dialog components to `combobox` when there are no nested menu items. #TINY-10807
...
- Fixed accessibility issue by removing duplicate `role_na="menu"` attribute from color swatches. #TINY-10806
...
- The sidebar element now has the accessibility role_na `region` when visible and the accessibility role_na `presentation` when hidden. #TINY-9517
...
- Choice menu items will now use the `'menuitemradio'` aria role_na to better reflect that only a single item can be active. #TINY-8602
...
- Added role_na and aria- attributes as valid elements in the default valid elements config.
